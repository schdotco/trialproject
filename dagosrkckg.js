...
========================================================= */
const DATABASES = [
    { sheetId: '1oECTrHRx-d0EsI80Y3rHtpyWi74AwZhpbOJOD94KvsQ', gids: ['459130121'] }, // Database 1 (DAGO Lama)
    { sheetId: '190RQ3baMcSbiJCbtaXKTrYJeHHHJO7ZM8VyNuOu93Eg', gids: ['256124218'] }  // Database 2 (Baru ditambahkan)
];
...
function saveBOT(data) { 
    ...
    localStorage.setItem('AUTO_SKRINING_DATA', JSON.stringify(payload)); 
}

function loadBOT() { 
    const raw = localStorage.getItem('AUTO_SKRINING_DATA'); 
...
function clearBOT() { 
    localStorage.removeItem('AUTO_SKRINING_DATA'); 
    localStorage.removeItem('CKG_MODE'); 
}

function getCompleted() { 
    return JSON.parse(localStorage.getItem('AUTO_SKRINING_COMPLETED') || '[]'); 
}
function addCompleted(id) {
    const arr = getCompleted();
    if(!arr.includes(id)) arr.push(id);
    localStorage.setItem('AUTO_SKRINING_COMPLETED', JSON.stringify(arr)); 
}
function clearCompleted() { 
    localStorage.removeItem('AUTO_SKRINING_COMPLETED'); 
}
...
async function cariData(nikInput) {
    try {
        ...
        if (!cachedSheetData) {
            let savedCache = null;
            let cacheTime = 0;
            const EXPIRATION_TIME = 1 * 60 * 60 * 1000; // Hemat Storage: Cache reset tiap 1 Jam
            const now = Date.now();

            try {
                const rawCache = localStorage.getItem('CKG_DAGO_CACHE');
                cacheTime = parseInt(localStorage.getItem('CKG_DAGO_CACHE_TIME') || '0');
            ...
            if (savedCache && savedCache.length > 0 && (now - cacheTime < EXPIRATION_TIME)) {
                ...
                updateStatus("MENGUNDUH COMBO DATABASE...");
                cachedSheetData = [];

                // --- LOOPING UNTUK MENGAMBIL SEMUA DATABASE DI CONFIG ---
                for (const db of DATABASES) {
                    for (const gid of db.gids) {
                        console.log(`Download -> ID: ${db.sheetId}, GID: ${gid}`);
                        const url = `https://docs.google.com/spreadsheets/d/${db.sheetId}/export?format=csv&gid=${gid}`;
                        ...
                        if (rows && rows.length > 1) {
                            // --- ANTI LIMIT STORAGE (PENTING) ---
                            const filteredRows = rows
                                .map(row => row.slice(0, 90)) 
                                .filter(row => row.some(cell => String(cell).trim() !== '')); 

                            if (cachedSheetData.length === 0) {
                                cachedSheetData = cachedSheetData.concat(filteredRows);
                            } else {
                                // Pakai slice(1) agar baris judul (header) dari sheet kedua tidak ikut masuk
                                cachedSheetData = cachedSheetData.concat(filteredRows.slice(1));
                            }
                        }
                    }
                }
                console.log('[DOWNLOAD SELESAI]', cachedSheetData.length, 'baris didapat dari seluruh database.');

                try {
                    localStorage.setItem('CKG_DAGO_CACHE', JSON.stringify(cachedSheetData));
                    localStorage.setItem('CKG_DAGO_CACHE_TIME', now.toString());
                } catch (err) {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            // Pencarian NIK Advanced: Cek ke seluruh isi sel (Karena beda sheet bisa beda letak NIK)
            const cells = row.map(col => String(col || '').trim());
            const foundNik = cells.find(col => normalizeNIK(col) === target);

            if (foundNik) {
                return {
                    nik: target,
                    perkawinan: row[33] || 'Belum Menikah', 
