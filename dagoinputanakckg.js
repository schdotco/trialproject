...
if (foundNik) {
                return {
                    ...
                    serumen: cells[11] || 'tidak ada',  // Kolom L (SERUMEN IMPAKSI)
                    infeksi_telinga: cells[12] || 'tidak ada', // Kolom M (INFEKSI TELINGA)
                    tajam_dengar: cells[13] || 'normal',// Kolom N (TAJAM PENDENGARAN)
...
async function handleTelingaMataAnak(data) {
    ...
    await isiDropdownSurveyJS('daya dengar', data.tajam_dengar);
    await sleep(800);

    await isiDropdownSurveyJS('daya lihat', data.tajam_lihat);
    await sleep(800);

    await isiDropdownSurveyJS('serumen impaksi', data.serumen);
    await sleep(800);

    await isiDropdownSurveyJS('infeksi telinga', data.infeksi_telinga);
    await sleep(800);

    // Untuk selaput mata merah, kita gunakan patokan dari kolom kacamata (Mata)
...
// RUTE 1: Telinga dan Mata (Anak Sekolah) -> Pakai Radio Button
        ...
        await pilihSemuaRadioLimit(data.tajam_dengar, 99, false); 
        await sleep(500);
        await pilihSemuaRadioLimit(data.tajam_lihat, 99, false); 
        await sleep(500);
        await pilihSemuaRadioLimit(data.serumen, 99, false); 
        await sleep(500);
        await pilihSemuaRadioLimit(data.infeksi_telinga, 99, false); 
        await sleep(800);
    }
    // RUTE 2: Telinga dan Mata (Balita / Prasekolah) -> Pakai Dropdown
        ...
        await selectDropdownSurveyJS(data.gigi);
    }
